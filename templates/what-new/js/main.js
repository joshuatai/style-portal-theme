...
$('.icon-down-arrow').on('click', function(e) {
    var nextTarget = $(e.target).parent().next();
    modalLgBody.mCustomScrollbar('scrollTo', nextTarget);
});
...
$(window).on('resize', function () {
    ...
    '.modal-lg .modal-body, .modal-md .modal-body { max-height: calc(' + ($(window).height() * 0.8) + 'px - 137px); }' + 
    '</style>').appendTo(head);
