...
(function($){
    ...
    $.fn.tagEditor = function(options, val, blur){
        ...
        if (typeof options == 'string') {
            ...
            selector.each(function(){
                ...
                if (options == 'getTags')
                    ...
                    // insert new tag
                    $('<li><div class="tag-editor-tag"></div><div class="tag-editor-delete"><span class="icon icon-cancel"></span></div></li>').appendTo(ed).find('.tag-editor-tag')
        ...
        return selector.each(function(){
            ...
            // markup for new tag
            if (o.clickEdit == false)
            var new_tag = '<li><div class="tag-editor-tag add-newtag"></div><div class="tag-editor-delete"><span class="icon icon-cancel"></span></div></li>';
            if (o.clickEdit == true)
            var new_tag = '<li><div class="tag-editor-tag tag-editor-editTag"></div><div class="tag-editor-delete"><span class="icon icon-cancel"></span></div></li>';
            
            // helper: update global data
            function set_placeholder(){
                if (o.placeholder && !tag_list.length && !$('.deleted, .placeholder, input', ed).length)
                    ed.append('<li class="placeholder">'+o.placeholder+'</li>');
            }
            ...
            // add a new tag when user click the input
            ed.click(function(e, closest_tag){
                ...
                if (o.clickEdit == false) {
                     if (loc == 'before') {
                        $(new_tag).insertBefore(closest_tag.closest('li')).find('.tag-editor-tag.add-newtag').click();
                    } else if (loc == 'after')
                        $(new_tag).insertAfter(closest_tag.closest('li')).find('.tag-editor-tag.add-newtag').click();
                    else // empty editor
                        $(new_tag).appendTo(ed).find('.tag-editor-tag.add-newtag').click();
                }
                else if (o.clickEdit == true) {
                    if (loc == 'before') {
                        $(new_tag).insertBefore(closest_tag.closest('li')).find('.tag-editor-tag.tag-editor-editTag').click();
                    } else if (loc == 'after')
                        $(new_tag).insertAfter(closest_tag.closest('li')).find('.tag-editor-tag.tag-editor-editTag').click();
                    else // empty editor
                        $(new_tag).appendTo(ed).find('.tag-editor-tag.tag-editor-editTag').click();
                }
                return false;
            });
            ...
            ed.on('click', '.tag-editor-tag.add-newtag', function(e){
                ...
                if (!$(this).hasClass('active')) {
                    ...
                    var left_percent = Math.abs(($(this).offset().left - e.pageX)/$(this).width()), caret_pos = parseInt(tag.length*left_percent),
                        input = $(this).html('<input type="text" maxlength="'+o.maxLength+'" value="'+escape(tag)+'">').removeClass('add-newtag').addClass('active').find('input');
            ...
            ed.on('click', '.tag-editor-tag.tag-editor-editTag', function(e){
                // delete on right click or ctrl+click -> exit
                if (o.clickDelete && (e.ctrlKey || e.which > 1)) return false;
                if (!$(this).hasClass('active')) {
                    var tag = $(this).text();
            ...
            function split_cleanup(input){
                ...
                for (var i=0; i<sub_tags.length; i++) {
                    ...
                    old_tags.push(tag);
                    li.before('<li><div class="tag-editor-tag">'+escape(tag)+'</div><div class="tag-editor-delete"><span class="icon icon-cancel"></span></div></li>');
            ...
            for (var i=0; i<tags.length; i++) {
                ...
                if (tag) {
                    ...
                    tag_list.push(tag);
                    if (o.clickEdit == false)
                        ed.append('<li><div class="tag-editor-tag">'+escape(tag)+'</div><div class="tag-editor-delete"><span class="icon icon-cancel"></span></div></li>');
                    if (o.clickEdit == true)
                        ed.append('<li><div class="tag-editor-tag tag-editor-editTag">'+escape(tag)+'</div><div class="tag-editor-delete"><span class="icon icon-cancel"></span></div></li>');
                }
            }
    ...
    $.fn.tagEditor.defaults = {
        ...
        clickDelete: false,
        clickEdit: false, //default can not edit tags
        animateDelete: 175,
