...
+(function (root, factory) {
    ...
    function Category(el, options) {
      ...
      if (this.options.type === 'sidebar') {
        ...
        this.$tabContainer = this.$el = $(el).addClass('sidebar').data('category', this);
    ...
    Category.prototype = {
      ...
      render: function () {
        ...
        this.selectedId ? showEvent.target = this.$links[this.selectedId][0] : showEvent.target = null;
      ...
      show: function (tab, index, duration) {
        ...
        if (options.animation === false) {
          ...
          if (this.$tabContainer.children().length === index + 1) {
            if (options.type !== 'sidebar') {
              this.$tabActivedHint.addClass('show');
            }
  
            var showEvent = $.Event('shown.bs.tab');
            showEvent.relatedTarget = null;
            if (this.selectedId) {
              showEvent.target = this.$links[this.selectedId][0];
            } else {
              showEvent.target = null;
            }
            this.$tabContainer.trigger(showEvent);
          }
        } else {
